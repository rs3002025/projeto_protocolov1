...
window.confirmarEncaminhamento = async function() {
  ...
  try {
    // ✨ OTIMIZADO: 'usuarioLogado' removido, o backend identifica pelo token
...
window.confirmarAtualizacaoStatus = async function() {
  ...
  try {
    // ✨ OTIMIZADO: 'usuarioLogado' removido
    const response = await fetchWithAuth(`/protocolos/atualizar`, {
      method: 'POST',
...
window.gerarImpressaoPersonalizada = async function() {
    ...
    if (filtrosHtml) filtrosHtml = filtrosHtml.slice(0, -2);

    let html = `
        <div style="text-align: center; margin-bottom: 5px; padding-bottom: 5px; border-bottom: 1px solid #ccc;">
            <img src="/img/logo.png" alt="Logo" style="height: 45px; margin-bottom: 5px;">
...
window.confirmarAlteracaoSenha = async function() {
    ...
    if (novaSenha.length < 4) { alert('A nova senha deve ter pelo menos 4 caracteres.'); return; }
    try {
        // ✅ CORRIGIDO: Removido 'usuarioLogin' do corpo da requisição
