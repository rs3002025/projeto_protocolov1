...
window.confirmarEncaminhamento = async function() {
  ...
  try {
    const response = await fetchWithAuth('/protocolos/atualizar', { method: 'POST', body: JSON.stringify({ protocoloId: idProtocolo, novoStatus: novoStatus, novoResponsavel: destino, observacao: `Encaminhado por ${window.usuarioLogado} para ${destino}` }) });
    const data = await response.json();
    if (data.sucesso) {
...
window.confirmarAtualizacaoStatus = async function() {
  ...
  try {
    const response = await fetchWithAuth(`/protocolos/atualizar`, { method: 'POST', body: JSON.stringify({ protocoloId: protocoloId, novoStatus: novoStatus, novoResponsavel: window.usuarioLogin, observacao: observacaoFinal }) });
    const data = await response.json();
    if (data.sucesso) {
