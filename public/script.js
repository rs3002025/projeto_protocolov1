...
    const inputAnexo = document.getElementById('novoAnexoInput');
    const modal = document.getElementById('modalAnexos');
    modal.dataset.protocoloId = protocoloId;
    listaAnexos.innerHTML = '<li>Carregando anexos...</li>';
    inputAnexo.value = null; // Limpa o input de arquivo
    try {
        const res = await fetch(`/protocolos/${protocoloId}/anexos`);
        const data = await res.json();
        listaAnexos.innerHTML = '';
        if (data.anexos && data.anexos.length > 0) {
            data.anexos.forEach(anexo => {
                const li = document.createElement('li');
                const dataCriacao = new Date(anexo.created_at).toLocaleDateString('pt-BR');
                const tamanho = (anexo.file_size / 1024 / 1024).toFixed(2);
                li.innerHTML = `
                    <span>${anexo.file_name} (${tamanho}MB) - ${dataCriacao}</span>
                    <div>
                        <button onclick="baixarAnexo(${anexo.id})">Baixar</button>
                        <button onclick="excluirAnexo(${anexo.id}, ${protocoloId})" style="background-color: #c82333;">Excluir</button>
                    </div>
                `;
                listaAnexos.appendChild(li);
            });
        } else {
            listaAnexos.innerHTML = '<li>Nenhum anexo encontrado.</li>';
        }
        modal.style.display = 'flex';
    } catch (err) {
...
    const protocoloId = document.getElementById('modalAnexos').dataset.protocoloId;
    const inputAnexo = document.getElementById('novoAnexoInput');
    if (!inputAnexo.files || inputAnexo.files.length === 0) { alert("Por favor, selecione um arquivo."); return; }
    const formData = new FormData();
    formData.append('anexo', inputAnexo.files[0]);
    try {
        const res = await fetch(`/protocolos/${protocoloId}/anexos`, { method: 'POST', body: formData });
        const data = await res.json();
        alert(data.mensagem);
        if (data.sucesso) { abrirModalAnexos(protocoloId); }
    } catch (err) { alert("Erro ao enviar anexo."); console.error(err); }
};
window.baixarAnexo = async function(anexoId) {
    try {
        const res = await fetch(`/protocolos/anexos/${anexoId}/download`);
        const data = await res.json();
        if (data.sucesso) { window.open(data.url, '_blank'); } 
        else { alert(data.mensagem || 'Erro ao gerar link de download.'); }
    } catch(err) { alert('Erro ao baixar anexo.'); console.error(err); }
};
window.excluirAnexo = async function(anexoId, protocoloId) {
    if (confirm("Tem certeza que deseja excluir este anexo?")) {
        try {
            const res = await fetch(`/protocolos/anexos/${anexoId}`, { method: 'DELETE' });
            const data = await res.json();
            alert(data.mensagem);
            if (data.sucesso) { abrirModalAnexos(protocoloId); }
        } catch (err) { alert("Erro ao excluir anexo."); console.error(err); }
    }
};
